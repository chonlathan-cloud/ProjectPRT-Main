...
export const Form: React.FC = () => {
  ...
  const handleSaveToBackend = async (autoGeneratedPdf?: File) => {
    // ---------------------------------------------------------
    ...
    if (data.type === 'pv' && !selectedPsFile && !autoGeneratedPdf) {
    ...
    try {
      ...
      const fileToUpload = autoGeneratedPdf || selectedPsFile;

      // Upload PS File if exists
      if (fileToUpload) {
        try {
          console.log(`Uploading PS file for Case ID: ${newCase.id}`);
          await uploadDocumentFile(newCase.id, fileToUpload);
        } catch (uploadError) {
      ...
      if (data.type === 'pv' && fileToUpload) {
        try {
          await uploadDocumentFile(newCase.id, fileToUpload, 'PS');
          if (!autoGeneratedPdf) setSelectedPsFile(null);
        } catch (error) {
  ...
  const generatePDF = async (action: 'submit' | 'download') => {
    ...
    try {
      ...
      for (let i = 0; i < elementsToCapture.length; i++) {
        ...
        // 1. Clone the element into an off-screen container at full A4 size
        //    This prevents the preview container's scale(0.9) and width constraints
        //    from compressing the text during capture
        ...
        // Let browser re-layout the clone
      ...
      if (action === 'download') {
        ...
        // Auto-generate blob for submission
        const pdfBlob = pdf.output('blob');
        const pdfFile = new File([pdfBlob], fileName, { type: 'application/pdf' });
        
        // Save to backend and upload automatically
        const success = await handleSaveToBackend(pdfFile);
        if (!success) return;
        
        // Optionally download it as well for their records
        pdf.save(fileName);
      }
    } catch (error) {
