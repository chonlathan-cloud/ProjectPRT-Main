...
export const Form: React.FC = () => {
  ...
  const generatePDF = async (action: 'submit' | 'download') => {
    ...
    try {
      ...
      const draftFileName = `${data.type}-draft.pdf`;
      ...
      if (action === 'download') {
        ...
        const pdfFile = new File([pdfBlob], draftFileName, { type: 'application/pdf' });
        
        // Save to backend and upload automatically
        const savedDocNo = await handleSaveToBackend(pdfFile);
        if (!savedDocNo) return;
        
        // Optionally download it as well for their records
        pdf.save(`${data.type}-${savedDocNo}.pdf`);
      }
    } catch (error) {
